...
class PrivateGPTOutputProcessor(BaseAppOutputsProcessor[PrivateGPTAppOutputs]):
    async def _generate_outputs(
        self,
        helm_values: dict[str, t.Any],
        app_instance_id: str,
    ) -> PrivateGPTAppOutputs:
        return PrivateGPTAppOutputs.model_validate(
            await get_privategpt_outputs(helm_values, app_instance_id)
        )
